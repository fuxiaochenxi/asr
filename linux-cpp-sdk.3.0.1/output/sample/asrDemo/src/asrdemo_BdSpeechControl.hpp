...
#ifndef ASRDEMO_ASRDEMO_BDSPEECHCONTROL_HPP_
#define ASRDEMO_ASRDEMO_BDSPEECHCONTROL_HPP_
#include <string>
...
#include "asrdemo_common.h"
#include "asrdemo_ResultListener.hpp"
namespace asrdemo {

class BdSpeechControl {
public:
    /**
     ...
     */
    BdSpeechControl(const std::string &app_name, const std::string &resource_path, ResultListener &listener);

    virtual ~BdSpeechControl();
    ...
    /**
     * 是否结束。
     * 收到SDK的3个空闲状态返回，认为识别过程结束，并且调用release。见output_callback对_is_finished的修改
     */
    bool is_finished();
...
private:
    ...
    DISALLOW_COPY_AND_ASSIGN(BdSpeechControl);
};
}/* namespace asrdemo */
