...
bool _audio_file_rate_is_8k = false; // 是否是8k采样率音频，8k = true， 16k = false；
...
static void set_config(bds::BDSSDKMessage &cfg_params) {
    ...
    if (_audio_file_rate_is_8k) { //8k采样率音频需要设置
...
static void _config(asrdemo::BdSpeechControl &controller);
// 按320字节大小，将数据post到sdk中。
static bool _post_data(istream &io, asrdemo::BdSpeechControl &controller, int &sleep_ms, string &error_msg);
// 判断流是否结束或异常(io.eof())，结束的话，调用post_data_finish_and_stop，告知sdk音频结束
static int _push_data_eof(istream &io, asrdemo::BdSpeechControl &controller, string &error_msg);
// flags 是否都是true
static bool _is_all_true(bool flags[], int size);

int main(int argc, char** argv) {
    cout << "BEGIN!" << endl;
    asrdemo::BdSpeechControl::open_log_file("asrdemo.log", 25); //此处10 表示日志文件为 25*512k大小，asrdemo.log超过大小后，自动重命名为asrdemo.log.bak。asrdemo.log继续生成
    //recog_one_file(); // 识别一个文件
    recog_multi_files(); // 一个线程里识别多个文件
    asrdemo::BdSpeechControl::close_log_file(); // 程序退出时或者不需要再写日志 固定用法， 引擎空闲时调用
    asrdemo::BdSpeechControl::do_cleanup(); // 程序退出时或者不需要asr服务时 固定用法， 引擎空闲时调用
    return 0;
}
...
static void recog_one_file() {
    ...
    asrdemo::BdSpeechControl controller("testYourApp", "../../../resources/asr_resource/", listener);
    _config(controller);
    ...
    int sleep_ms = 0;
    while (will_continue) {
        status = _push_data_eof(io, controller, error_msg); // 判断是否是文件结尾，是的话，结束这个音频文件的SDK调用
        if (status < 0) { // 出错
...
static void recog_multi_files() {
    ...
    asrdemo::BdSpeechControl *controllers[num_files]; // BdSpeechControl 控制类
    ...
    for (int i = 0; i < num_files; i++) {
        ...
        controllers[i] = new asrdemo::BdSpeechControl("testAppM", "../../../resources/asr_resource/", *listeners[i]);
...
static bool _post_data(istream &io, asrdemo::BdSpeechControl &controller, int &sleep_ms, string &error_msg) {
    ...
    if (_audio_file_rate_is_8k) {
        sleep_ms = asrdemo::Util::cal_speech_8k_duration_ms(readed_len); // 根据audio_buf_len 计算音频长度 8k 采样率
    } else {
