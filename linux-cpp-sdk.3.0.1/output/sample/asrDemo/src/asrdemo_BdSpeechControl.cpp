...
#include "asrdemo_BdSpeechControl.hpp"
namespace asrdemo {
...
BdSpeechControl::BdSpeechControl(const std::string &app_name, const std::string &resource_path,
        ResultListener &listener) :
        _app_name(app_name), _resource_path(resource_path), _listener(listener), _is_inited(false), _is_finished(false), _has_error(
                false) {
...
bool BdSpeechControl::config(bds::BDSSDKMessage & config_params, std::string &error_msg) {
    if (_is_inited) {
        _error_msg = "BdSpeechControl has already initialized";
    }
...
bool BdSpeechControl::post_audio_data(const char *audio_buf, int audio_buf_len, std::string &error_msg) {
...
bool BdSpeechControl::post_data_finish_and_stop(std::string &error_msg) {
    if (!BdSpeechControl::post_audio_data(NULL, 0, error_msg)) {
        return false;
    }
...
bool BdSpeechControl::cancel(std::string &error_msg) {
...
bool BdSpeechControl::stop(std::string &error_msg) {
...
void BdSpeechControl::output_callback(bds::BDSSDKMessage& message, void* user_arg) {
    ...
    BdSpeechControl * controller = (BdSpeechControl *) user_arg;
    ...
    if (is_finished) {
        ...
        controller->_is_finished = true;
        pthread_mutex_unlock(&controller->thread_mutex);
    }

}

void BdSpeechControl::set_config_params(bds::BDSSDKMessage &config_params) {
    config_params.name = bds::ASR_CMD_CONFIG;
    std::string temp_str;

    // 校验必填字段
    const std::string required_string_fields[] = { bds::ASR_PARAM_KEY_APP_ID, bds::ASR_PARAM_KEY_CHUNK_KEY,
            bds::ASR_PARAM_KEY_SECRET_KEY, bds::ASR_PARAM_KEY_PRODUCT_ID };
    std::string key;
    ...
    if (!config_params.get_parameter(bds::ASR_PARAM_KEY_MFE_DNN_DAT_FILE, temp_str)) {
        config_params.set_parameter(bds::ASR_PARAM_KEY_MFE_DNN_DAT_FILE, _resource_path + "bds_easr_mfe_dnn.dat");
    }
    if (!config_params.get_parameter(bds::ASR_PARAM_KEY_MFE_CMVN_DAT_FILE, temp_str)) {
        config_params.set_parameter(bds::ASR_PARAM_KEY_MFE_CMVN_DAT_FILE, _resource_path + "bds_easr_mfe_cmvn.dat");
    }
    int tmp_int;
    if (!config_params.get_parameter(bds::COMMON_PARAM_KEY_DEBUG_LOG_LEVEL, tmp_int)) {
        config_params.set_parameter(bds::COMMON_PARAM_KEY_DEBUG_LOG_LEVEL, bds::EVRDebugLogLevelTrace);
    }
    _has_error = !_sdk->post(config_params, _error_msg);
}

void BdSpeechControl::set_start_params() {
...
bool BdSpeechControl::is_finished() {
    bool is_finished;
...
BdSpeechControl::~BdSpeechControl() {
