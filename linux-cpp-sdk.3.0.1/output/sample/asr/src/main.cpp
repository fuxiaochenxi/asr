...
void asr_set_config_params(bds::BDSSDKMessage& cfg_params) {
    ...
    cfg_params.set_parameter(bds::ASR_PARAM_KEY_SECRET_KEY, secret_key);
    // cfg_params.set_parameter(bds::ASR_PARAM_KEY_SAMPLE_RATE, bds::EVoiceRecognitionRecordSampleRate8K); //8k采样率的音频需要设置
...
int asr_online_pushaudio(const char* file_path, const std::string& push_cmd, bds::BDSpeechSDK* sdk, int thread_seq) {
    ...
    const double per_send_seconds = (sample_rate == 16000) ? 0.01 : 0.02; //320字节 ，16000采样率 sleep 10ms，8000采样率停 sleep 20ms // sleep时间与包大小成正比
...
void asr_output_callback(bds::BDSSDKMessage& message, void* user_arg) {
    ...
    switch (status) {
        ...
        case bds::EVoiceRecognitionClientWorkStatusFlushData: { // 连续上屏,中间结果
            ...
            fprintf(result_output_file, "patial result: %s\n", json_result.c_str());
            fflush(result_output_file);
            break;
        }
        ...
        case bds::EVoiceRecognitionClientWorkStatusChunkNlu: { //语义解析
            ...
            fprintf(result_output_file, "nlu result:");
            for (int i = 0; i < len; ++i) {
